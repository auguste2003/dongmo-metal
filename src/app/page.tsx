          ...
          <div className="grid md:grid-cols-2 gap-8">
            {navLinks.map((link) => (
              <Link href={link.href} key={link.href} className="group">
                <Card className="h-full p-8 flex flex-col items-center text-center shadow-lg hover:shadow-2xl hover:-translate-y-2 transition-transform duration-300">
                  <link.icon className="h-16 w-16 text-accent mb-4" />
                  <CardTitle className="text-2xl font-bold">{link.title}</CardTitle>
                  <CardDescription className="mt-2 text-lg">{link.description}</CardDescription>
                  <ArrowRight className="mt-4 h-6 w-6 text-accent opacity-0 group-hover:opacity-100 transition-opacity" />
                </Card>
              </Link>
            ))}
          </div>
